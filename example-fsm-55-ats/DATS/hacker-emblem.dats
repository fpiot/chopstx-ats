...
#define DATA55(A,B,C,D,E) (int2char0 A, int2char0 B, int2char0 C, int2char0 D, int2char0 E)
val image0 = $arrpsz{l55} (
  DATA55(0x00,0x00,0x00,0x00,0x00),
  DATA55(0x00,0x01,0x00,0x00,0x00),
  DATA55(0x01,0x03,0x01,0x01,0x00),
  DATA55(0x02,0x06,0x02,0x02,0x01),
...
implement main () = {
  fun loop (state: uint): uint = nstate where {
    val nstate = c_main (state)
  }

  fun forever (state: uint): void = {
    val nstate = loop (state)
    val () = forever (nstate)
  }
